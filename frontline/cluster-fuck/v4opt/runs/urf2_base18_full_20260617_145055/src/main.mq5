...
input double URF_BaseScale          = 1.8;    // default lot multiplier (was blind 2.5)
...
double United_RiskFacadeScale()
{
   ...
   // Start from the tunable base scale; protections only throttle it DOWN.
   double scale = URF_BaseScale;

   // A. Margin-load cap: keep used-margin / equity <= URF_MaxMarginLoadPct.
   if(loadPct > URF_MaxMarginLoadPct)
      scale = MathMin(scale, URF_BaseScale * URF_MaxMarginLoadPct / MathMax(loadPct, 0.01));

   // B. Margin-level floor: throttle only in a TIGHT band just above the
   //    300% floor (zero new exposure at/below the floor). Band = floor*1.15.
   double floorBand = URF_MinMarginLevelPct * 1.15;
   if(level < floorBand)
   {
   ...
   // C. Equity-peak drawdown breaker: multiplicative throttle from soft to hard DD.
