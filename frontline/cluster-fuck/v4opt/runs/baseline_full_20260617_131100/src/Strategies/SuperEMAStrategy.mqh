...
#endif
      if(!United_MayOpenNewEntry(d.symbol, (ulong)d.magic, true))
         return;
      if(d.trade.Buy(vol, d.symbol, tick.ask, sl, tp, "United SuperEMA long #M" + IntegerToString(d.magic)))
...
#endif
      if(!United_MayOpenNewEntry(d.symbol, (ulong)d.magic, false))
         return;
      if(d.trade.Sell(vol, d.symbol, tick.bid, sl, tp, "United SuperEMA short #M" + IntegerToString(d.magic)))
