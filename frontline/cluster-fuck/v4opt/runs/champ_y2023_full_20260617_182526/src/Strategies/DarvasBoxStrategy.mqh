...
bool PlaceOrder(ENUM_ORDER_TYPE orderType, double price, double sl, double tp)
{
   ...
   if(orderType == ORDER_TYPE_BUY)
   {
      if(!United_MayOpenNewEntry(dbData.symbol, (ulong)DB_MagicNumber, true))
         return false;
      result = dbData.trade.Buy(lot, dbData.symbol, 0, sl, tp, "Darvas Box Breakout #M" + IntegerToString(DB_MagicNumber));
   }
   else
   {
      if(!United_MayOpenNewEntry(dbData.symbol, (ulong)DB_MagicNumber, false))
         return false;
      result = dbData.trade.Sell(lot, dbData.symbol, 0, sl, tp, "Darvas Box Breakdown #M" + IntegerToString(DB_MagicNumber));
   }
