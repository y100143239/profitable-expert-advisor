...
// V4-iter8-E2: HTF (D1 EMA200) trend veto for RRA SELL entries.
// Inputs RRA_VetoSellAgainstHTF, RRA_HTF_TimeFrame, RRA_HTF_EMAPeriod, RRA_HTF_SlopeBars
// are declared as input vars in main.mq5 and are globally visible to all included files.

bool RRA_HTF_IsTrendUp(const string sym)
{
   static string s_sym[8];
   static int    s_handle[8] = {-1,-1,-1,-1,-1,-1,-1,-1};
   static int    s_period[8] = {0,0,0,0,0,0,0,0};
   static int    s_tf[8]     = {0,0,0,0,0,0,0,0};
   int slot = -1;
   for(int i=0;i<8;i++){
      if(s_handle[i]!=-1 && s_sym[i]==sym && s_period[i]==RRA_HTF_EMAPeriod && s_tf[i]==(int)RRA_HTF_TimeFrame){ slot=i; break; }
   }
   if(slot==-1){
      for(int i=0;i<8;i++){ if(s_handle[i]==-1){ slot=i; break; } }
      if(slot==-1) return false;
      int h = iMA(sym, RRA_HTF_TimeFrame, RRA_HTF_EMAPeriod, 0, MODE_EMA, PRICE_CLOSE);
      if(h==INVALID_HANDLE) return false;
      s_sym[slot]=sym; s_handle[slot]=h; s_period[slot]=RRA_HTF_EMAPeriod; s_tf[slot]=(int)RRA_HTF_TimeFrame;
   }
   double buf[];
   int need = RRA_HTF_SlopeBars + 2;
   if(CopyBuffer(s_handle[slot], 0, 0, need, buf) < need) return false;
   ArraySetAsSeries(buf, true);
   double emaNow  = buf[1];                    // last fully-closed bar
   double emaPast = buf[1 + RRA_HTF_SlopeBars];
   double price   = SymbolInfoDouble(sym, SYMBOL_BID);
   return (emaNow > emaPast && price > emaNow);
}
...
void ProcessRSIReversalAsian(RSIReversalAsianData& data, double lotSize)
{
   ...
   if(!hasOpenPosition)
   {
      ...
      if(data.rsiCrossedOversold)
      {
         ...
         if(!United_MayOpenNewEntry(data.symbol, (ulong)data.MagicNumber, true))
            return;

         if(data.trade.Buy(vol, data.symbol, currentAsk, sl, tp, "RSI Oversold Crossover Buy #M" + IntegerToString(data.MagicNumber)))
         {
      ...
      else if(data.rsiCrossedOverbought)
      {
         ...
         // V4-iter8-E2: HTF (D1 EMA200) trend veto - block SELL when daily trend is up.
         if(RRA_VetoSellAgainstHTF && RRA_HTF_IsTrendUp(data.symbol))
            return;

         if(!United_MayOpenNewEntry(data.symbol, (ulong)data.MagicNumber, false))
            return;

         if(data.trade.Sell(vol, data.symbol, currentBid, sl, tp, "RSI Overbought Crossover Sell #M" + IntegerToString(data.MagicNumber)))
         {
            data.isPositionOpen = true;
