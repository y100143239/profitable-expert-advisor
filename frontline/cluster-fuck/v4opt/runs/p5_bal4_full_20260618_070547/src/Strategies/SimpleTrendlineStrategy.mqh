...
void ST_TryExitOnBreak(SimpleTrendlineData &d, const SimpleTrendlineModel &m)
{
   ...
   // V4-iter5: time-based forced exit for stuck positions (per-magic gate).
   if(ST_MaxHoldHours > 0 && (ST_MaxHoldMagic == 0 || ST_MaxHoldMagic == d.magic || ST_MaxHoldMagic2 == d.magic))
   {
      datetime openT = (datetime)PositionGetInteger(POSITION_TIME);
      if(openT > 0 && (TimeCurrent() - openT) >= (long)ST_MaxHoldHours * 3600)
      {
   ...
   // V4-iter8-H: MinHold gate — block early trendline-break exit (does not affect MaxHold cap above).
   if(closePos && ST_MinHoldHours > 0 && ST_MinHoldMagic != 0 && ST_MinHoldMagic == d.magic)
   {
      datetime openT = (datetime)PositionGetInteger(POSITION_TIME);
      if(openT > 0 && (TimeCurrent() - openT) < (long)ST_MinHoldHours * 3600)
         closePos = false;
   }
...
bool ST_TryEarlyFailureExit(SimpleTrendlineData &d, const SimpleTrendlineModel &m)
{
   if(!ST_EarlyFailureExitEnable)
      return false;
   ...
   int periodSeconds = PeriodSeconds(d.signalTF);
   if(periodSeconds <= 0)
      return false;

   long heldSeconds = (long)(TimeCurrent() - openTime);
   if(ST_EarlyFailureMaxHoldHours > 0 && heldSeconds > (long)ST_EarlyFailureMaxHoldHours * 3600)
      return false;
   if(ST_EarlyFailureMinHoldBars > 0 && heldSeconds < (long)ST_EarlyFailureMinHoldBars * periodSeconds)
      return false;

   double close1 = iClose(d.symbol, d.signalTF, 1);
   datetime closeTime1 = iTime(d.symbol, d.signalTF, 1);
   if(close1 <= 0.0 || closeTime1 <= 0)
      return false;
   ...
   double line1 = ST_LinePriceAt(m, closeTime1);
   double lossPoints = 0.0;
   bool failedLineSide = false;

   if(posType == POSITION_TYPE_BUY)
   {
      lossPoints = (openPrice - close1) / point;
      failedLineSide = (close1 <= line1);
   }
   else if(posType == POSITION_TYPE_SELL)
   {
      lossPoints = (close1 - openPrice) / point;
      failedLineSide = (close1 >= line1);
   }
   else
      return false;

   if(lossPoints < ST_EarlyFailureMinLossPoints)
      return false;
   if(lossPoints <= 0.0 || !failedLineSide)
      return false;
...
void ST_TryPullbackEntry(SimpleTrendlineData &d, const SimpleTrendlineModel &m, const double lots)
{
   ...
   if(ST_MinSlopePointsPerBar > 0.0)
   {
      double point = SymbolInfoDouble(d.symbol, SYMBOL_POINT);
      double slopePoints = (point > 0.0 ? MathAbs(m.a * PeriodSeconds(d.signalTF)) / point : 0.0);
      if(slopePoints < ST_MinSlopePointsPerBar)
         return;
   }
   ...
   if(upTrend)
   {
      ...
      if(touched && reclaim && bullish && stillHealthy)
      {
         if(!United_MayOpenNewEntry(d.symbol, d.magic, true))
            return;
         if(!d.trade.Buy(vol, d.symbol, 0.0, 0.0, 0.0, "SimpleTrendline BUY #M" + IntegerToString(d.magic)))
   ...
   else if(downTrend)
   {
      ...
      if(touched && reject && bearish && stillWeak)
      {
         if(!United_MayOpenNewEntry(d.symbol, d.magic, false))
            return;
         if(!d.trade.Sell(vol, d.symbol, 0.0, 0.0, 0.0, "SimpleTrendline SELL #M" + IntegerToString(d.magic)))
