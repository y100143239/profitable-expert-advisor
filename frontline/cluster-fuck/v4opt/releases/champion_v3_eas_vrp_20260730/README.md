...
| 2026-06-01–0730 (近期/对比实盘) | +440 | 19.49% | 25.94% | 1.15 | 232 |

- 四个年度窗口 **全部盈利**，盈利因子 1.08–1.48；2023 为最弱窗口（边际优势较薄、回撤较高）。
- **近期 6/1–7/30**：仍盈利 +440（PF 1.15），但 净值回撤 25.94% 较颇籍。归因：**XAUUSD（黄金）本区间 -240**（10% 胜率）为主要拖累；
  黄金具有强周期性（在 2024/2025/2026 全年为净盈利），因此**不建议删除黄金子策略**；这是黄金为重的组合在个别难做区间的固有现象，非策略错误。
...
**本版 ex5 已将最优参数烧入默认值**（`EnableRSIScalpingMU=false`、`EAS_Enable=true`、`VRP_Enable=true`），
其余参数与冠军配置一致。因此：

1. 将 `main.ex5` 放入 MT5 的 `MQL5\Experts\` 目录（或用 `src\main.mq5` 在 MetaEditor 中重新编译）。
2. 直接将 EA 挂到图表即为最优配置（首次挂载使用编译默认值）。
3. **为保险起见（强烈建议）**：在 EA 参数窗口点 **Load** 加载 `champion_v3.set` 一次。
   原因：MT5 的图表/测试器可能缓存“上次使用的参数”，加载 `.set` 可强制重置为最优值。
4. 建议测试/运行环境：EURUSD 图表挂载、Deposit 3000、Leverage 1000、Model=4（真实 Tick）。
