...
void ProcessRSIConsolidation(RSIConsolidationData &d, const double lots)
{
   ...
   if(RCO_EntryBuyCross(d, rsi2, rsiPrev))
   {
      ...
      if(!d.trade.Buy(vol, d.symbol, ask, sl, tp, "RSIConsolidation BUY #M" + IntegerToString(d.magic)))
   ...
   else if(RCO_EntrySellCross(d, rsi2, rsiPrev))
   {
      ...
      sl = NormalizeDouble(sl, dig);
      tp = NormalizeDouble(tp, dig);
      if(!d.trade.Sell(vol, d.symbol, bid, sl, tp, "RSIConsolidation SELL #M" + IntegerToString(d.magic)))
