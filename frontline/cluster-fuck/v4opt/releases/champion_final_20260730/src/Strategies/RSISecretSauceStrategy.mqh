...
void RSS_OpenPosition(RSISecretSauceOrcData &d, ENUM_POSITION_TYPE type, const double lotSize)
{
   ...
   string comment = "RSI_Secret_" + (type == POSITION_TYPE_BUY ? "LONG" : "SHORT") + " #M" + IntegerToString(RSS_MagicNumber);

   bool result = false;
   if(type == POSITION_TYPE_BUY)
   {
      if(!United_MayOpenNewEntry(d.actualSymbol, (ulong)RSS_MagicNumber, true))
         return;
      result = d.trade.Buy(lotSize, d.actualSymbol, 0, sl, tp, comment);
   }
   else
   {
      if(!United_MayOpenNewEntry(d.actualSymbol, (ulong)RSS_MagicNumber, false))
         return;
