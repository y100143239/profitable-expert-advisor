...
void CheckExistingPosition(RSIScalpingData& data, ENUM_TIMEFRAMES TimeFrame, int MagicNumber,
                          ...
                          double RSI_Target_Sell, int BarsToWait, bool UseClosedBarExit)
{
   ...
   double exitRsi = (UseClosedBarExit ? data.rsi_prev : data.rsi_current);

   if(data.current_position_type == POSITION_TYPE_BUY)
   {
      if(exitRsi < RSI_Oversold)
      {
...
void CheckEntrySignals(RSIScalpingData& data, ENUM_TIMEFRAMES TimeFrame, int MagicNumber,
                      ...
                      bool RequireOrderedBands)
{
   if(RequireOrderedBands && RSI_Overbought <= RSI_Oversold)
      return;
...
{
   if(!United_MayOpenNewEntry(data.symbol, (ulong)MagicNumber, true))
      return;
   ...
   if(data.trade.Buy(normalizedLot, data.symbol, ask, 0, 0, "RSI Scalping Buy #M" + IntegerToString(MagicNumber)))
   {
...
{
   if(!United_MayOpenNewEntry(data.symbol, (ulong)MagicNumber, false))
      return;
   ...
   if(data.trade.Sell(normalizedLot, data.symbol, bid, 0, 0, "RSI Scalping Sell #M" + IntegerToString(MagicNumber)))
   {
      ulong new_ticket = data.trade.ResultOrder();
      if(new_ticket > 0)
      {
                       ...
                       bool UseTrailingStop, double TrailingStopDistancePoints, double TrailingActivationPoints)
{
   ...
   CheckExistingPosition(data, TimeFrame, MagicNumber, RSI_Oversold, RSI_Overbought,
                        RSI_Target_Buy, RSI_Target_Sell, BarsToWait, UseClosedBarExit);
   
   if(!data.position_open && !PositionExistsByMagic(data.symbol, MagicNumber))
   {
      CheckEntrySignals(data, TimeFrame, MagicNumber, RSI_Oversold, RSI_Overbought, LotSize, RequireOrderedBands);
   }
}
