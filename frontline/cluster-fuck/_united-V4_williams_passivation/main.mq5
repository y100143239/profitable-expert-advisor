...
bool United_IsKnownV4Magic(const ulong magic)
{
   return magic == 135790      // DarvasBox
       ...
       || magic == 1001 || magic == 1002 || magic == 1003 // RSI MidPoint Hijack
       || magic == 20001 || magic == 123459123 || magic == 20003 || magic == 125421321 || magic == 129102315 // RSI Scalping
       || magic == 30001 || magic == 30002 // RSI Reversal Asian
       || magic == 940001 // SuperEMA
       || magic == 20250420 // RSI Consolidation
       || magic == 26042501 || magic == 26042502 || magic == 26042503 // Simple Trendline
       || magic == 789012 // RSI Secret Sauce
       || magic == 20260524; // Williams Passivation
}
...
{
   if(!GRM_ConsecutiveLossCooldownEnable || GRM_ConsecutiveLossCooldownBars <= 0)
      return false;
   ...
   {
      Print("GRM blocks entry: consecutive-loss cooldown, symbol=", symbol, ", magic=", magic, ", losses=", losses);
      return true;
   }
   return false;
}
...
input group "=== Consecutive Loss Cooldown ==="
input bool   GRM_ConsecutiveLossCooldownEnable = true;
...
void United_RefreshScaledLots()
{
   ...
   g_WP_LotSize = United_ScaledLot(LOT_WP_WilliamsPassivation);
}
