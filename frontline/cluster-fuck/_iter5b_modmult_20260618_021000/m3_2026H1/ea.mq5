...
// iter3: hard margin-safety guard (prevents the 2026H1 stop-out blowup)
input bool   UseMarginGuard     = true;   // Clamp reverse lot to respect margin limits
input double minMarginLevelPct  = 300.0;  // Never let post-trade margin level fall below this
input double maxMarginLoadPct   = 25.0;   // Never let total used margin exceed this % of equity

// iter4: hard money stop on the open book (出现亏损后及时止损).
// Universal safety net that bounds the worst-case floating loss regardless of
// which mechanism (fast move OR lone-position float) is causing it. This is the
// only fix that prevents BOTH blowup modes (2026H1 fast stop-out + 2025 lone float).
input bool   UseHardMoneyStop   = true;   // Close the whole book if floating loss breaches the cap
input double reverseStopLossUSD = 500.0;  // Max aggregate floating loss (USD) before forced close
...
//+------------------------------------------------------------------+
//| iter3: clamp a desired lot so opening it keeps margin level >=    |
//| minMarginLevelPct and total margin load <= maxMarginLoadPct.      |
//| Returns the largest safe lot (>=0). Prevents margin stop-outs.    |
//+------------------------------------------------------------------+
double MarginSafeLot(ENUM_ORDER_TYPE orderType, double desiredLot)
{
    if(!UseMarginGuard) return desiredLot;
    ...
    if(!OrderCalcMargin(orderType, Symbol(), 1.0, price, marginPerLot) || marginPerLot <= 0)
        return desiredLot; // cannot evaluate; fall back

    // Cap 1: total used margin <= maxMarginLoadPct% of equity
    double allowedTotalMargin_load = equity * (maxMarginLoadPct / 100.0);
    // Cap 2: margin level = equity / totalMargin * 100 >= minMarginLevelPct
    //        => totalMargin <= equity / (minMarginLevelPct/100)
    double allowedTotalMargin_level = equity / (minMarginLevelPct / 100.0);
    double allowedTotalMargin = MathMin(allowedTotalMargin_load, allowedTotalMargin_level);

    double freeForNew = allowedTotalMargin - usedMargin;
    if(freeForNew <= 0) return 0.0;

    double maxLot = freeForNew / marginPerLot;
    double safe = MathMin(desiredLot, maxLot);

    // round down to lot step
    double step = SymbolInfoDouble(Symbol(), SYMBOL_VOLUME_STEP);
    if(step > 0) safe = MathFloor(safe / step) * step;
    double minVol = SymbolInfoDouble(Symbol(), SYMBOL_VOLUME_MIN);
    if(safe < minVol) safe = 0.0; // can't even afford the minimum safely -> skip
    return NormalizeDouble(safe, 2);
}
...
//+------------------------------------------------------------------+
void OnTick() {
    // iter4: hard money stop runs FIRST, every tick, before any early return.
    // CRITICAL: after a reverse trade, crossoverTradeCount is set to
    // maxCrossoverTrades+1, which makes OnTick return early (below) before
    // CheckPositions() — leaving the reverse position unmanaged and free to
    // float into a margin stop-out. Enforcing the stop here guarantees the
    // worst-case aggregate floating loss is always bounded (出现亏损后及时止损).
    if (UseHardMoneyStop && reverseStopLossUSD > 0 && PositionsTotal() > 0) {
        double bookFloating = 0.0;
        for (int j = PositionsTotal() - 1; j >= 0; j--) {
            ulong t = PositionGetTicket(j);
            if (PositionSelectByTicket(t))
                bookFloating += PositionGetDouble(POSITION_PROFIT)
                              + PositionGetDouble(POSITION_SWAP);
        }
        if (bookFloating <= -reverseStopLossUSD) {
            CloseAllPositions();
            Print("Hard money stop hit: floating ", bookFloating,
                  " <= -", reverseStopLossUSD, " -> book closed");
            return;
        }
    }
                ...
                if (PositionSelectByTicket(ticket)) {
                    ...
                    if (tradeLength > tradeLengthThreshold * 60) { // Convert threshold to seconds
                        ...
                        if (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY) {
                            trade.SetExpertMagicNumber(MagicNumber);
                            double safeLot = MarginSafeLot(ORDER_TYPE_SELL, newLotSize);
                            if (safeLot <= 0) {
                                Print("Reversal sell skipped: margin guard (level<", minMarginLevelPct, "% or load>", maxMarginLoadPct, "%)");
                            } else if (trade.Sell(safeLot, Symbol(), SymbolInfoDouble(Symbol(), SYMBOL_BID))) {
                                Print("Reversal sell order executed (margin-safe lot ", safeLot, " of ", newLotSize, ")");
                            } else {
                                Print("Failed to execute reversal sell order");
                            }
                        } else if (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_SELL) {
                            trade.SetExpertMagicNumber(MagicNumber);
                            double safeLot = MarginSafeLot(ORDER_TYPE_BUY, newLotSize);
                            if (safeLot <= 0) {
                                Print("Reversal buy skipped: margin guard (level<", minMarginLevelPct, "% or load>", maxMarginLoadPct, "%)");
                            } else if (trade.Buy(safeLot, Symbol(), SymbolInfoDouble(Symbol(), SYMBOL_ASK))) {
                                Print("Reversal buy order executed (margin-safe lot ", safeLot, " of ", newLotSize, ")");
                            } else {
                    ...
                    // Check if there is only one position and it is the (large) reverse
                    // position left alone. iter4: compare against the live base lot instead
                    // of the exact value minimumLotSize*reverseLotSizeMultiplier, because the
                    // margin guard may clamp the reverse below that exact value (the old ==
                    // check then never matched, leaving a lone reverse to float to a
                    // stop-out -> the 2025 blowup). A fresh original entry has volume == base
                    // lot, so volume > base*1.5 reliably identifies a reverse only.
                    double baseLotNow = CalculateLotSize();
                    if (PositionsTotal() == 1 &&
                        PositionGetDouble(POSITION_VOLUME) > baseLotNow * 1.5) {
                        trade.PositionClose(ticket);
                        Print("Single (reverse) position closed");
                    }
