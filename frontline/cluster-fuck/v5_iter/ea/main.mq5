...
#property strict
#property description "V4.11 live-safe iteration: partial TP + leverage-aware sizing + anti-reentry (all opt-in). Base = champion_final_20260731."

// Human-readable build tag printed at init so the running binary is always identifiable in logs/journal.
#define EA_VERSION "4.11-dev"
...
// than a clock. Default OFF: on full-window real ticks (compounding) VRP inflates
// drawdown badly; opt-in only. Takes precedence over the timed cooldown when enabled.
...
// from RQE (counter-regime only) and SPTE (slow time-exit). Default OFF (opt-in; inert on full-window backtest).
...
{
   if(!GRM_LossHaltEnable) return 0.0;   // v4.11: loss-halt disabled by default (no forced stop)
...
{
   if(!GRM_LossHaltEnable) return 0.0;   // v4.11: loss-halt disabled by default (no MONTH-LOCK)
