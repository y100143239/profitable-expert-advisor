...
//+------------------------------------------------------------------+
// Rebuild a starved/invalidated D1 EMA handle in place. Live terminals can drop
// long-lived handles after reconnects/history refreshes, which used to hard-fail
// the strategy ("CopyBuffer D1 EMA failed"). Returns true if the handle is usable.
bool WP_RebuildD1EMAHandle(WilliamsPassivationData& data, const int EMA_Period, const ENUM_APPLIED_PRICE EMA_AppliedPrice)
{
   if(data.ema_handle != INVALID_HANDLE)
      IndicatorRelease(data.ema_handle);
   data.ema_handle = iMA(data.symbol, PERIOD_D1, EMA_Period, 0, MODE_EMA, EMA_AppliedPrice);
   return (data.ema_handle != INVALID_HANDLE);
}
                                ...
                                bool ExitOnPassivationEnd, double StopLossPoints, double TakeProfitPoints)
{
   ...
   // Check D1 200 EMA Trend. A live terminal can starve/invalidate a long-lived D1
   // handle; rebuild once, then fall back to the forming bar, instead of hard-
   // returning (which silently disabled the strategy live on 2026.07.13).
   double ema_d1[];
   ArraySetAsSeries(ema_d1, true);
   bool ema_ok = (CopyBuffer(data.ema_handle, 0, 1, 2, ema_d1) == 2);
   if(!ema_ok && WP_RebuildD1EMAHandle(data, EMA_Period, EMA_AppliedPrice))
   ...
   bool d1_ok = (CopyClose(symbol, PERIOD_D1, 1, 1, d1_close_array) == 1);
   if(!d1_ok)
      d1_ok = (CopyClose(symbol, PERIOD_D1, 0, 1, d1_close_array) == 1);
   if(!d1_ok)
      return;
