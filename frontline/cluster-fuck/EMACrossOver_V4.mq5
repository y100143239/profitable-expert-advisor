//+------------------------------------------------------------------+
//|                                             EMACrossOver_V4.mq5    |
//|   iter4 keeper engine + EQUITY-GROWTH lot booster.               |
//|   净值>余额时增大仓位(保证金率), 上限25%load / 下限300%level.      |
//+------------------------------------------------------------------+
#property strict
...
// ---- V4 equity-growth lot booster ----
input bool   UseEquityGrowthBoost = true;  // Grow lots when equity>balance
input double MaxGrowthMult        = 3.0;   // Cap on growth multiplier
input double GrowthSensitivity    = 1.0;   // 1.0 = linear with equity/balance
...
//+------------------------------------------------------------------+
//| Lot sizing: balance-base * equity-growth boost, then margin CAP   |
//+------------------------------------------------------------------+
double CalculateLotSize() {
    ...
    // 净值大于余额时增大仓位 (equity-growth booster)
    if (UseEquityGrowthBoost && balance > 0 && equity > balance) {
        double growth = 1.0 + (equity / balance - 1.0) * GrowthSensitivity;
        if (growth > MaxGrowthMult) growth = MaxGrowthMult;
        lotSize *= growth;
    }

    // Margin cap (25% load / 300% level)
    if (UseDynamicMargin) {
        ...
        if (OrderCalcMargin(ORDER_TYPE_BUY, Symbol(), 1.0, SymbolInfoDouble(Symbol(), SYMBOL_ASK), marginReq) && marginReq > 0) {
            ...
            if (capLot < lotSize) lotSize = capLot;
        }
    }
