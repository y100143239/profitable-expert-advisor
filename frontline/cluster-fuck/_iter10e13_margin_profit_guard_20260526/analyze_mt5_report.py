...
def analyze_mt5_deals(file_path):
    ...
    md_lines = []
    md_lines.append(f"# 🚨 策略真实净盈亏排行榜 & MT5 操作指南 🚨\n")
    md_lines.append(f"**分析时间:** {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    ...
    for strategy, profit in sorted_profits:
        mt5_param = mt5_mapping.get(strategy, '未知开关 (需查源码)')
    ...
    print("🛠️ 【MT5 傻瓜式斩首执行清单】")
    print("请回到 MT5 的 'Inputs (输入)' 面板，双击以下开关将其设为 false：")
    md_lines.append("## 🛠️ 【MT5 傻瓜式斩首执行清单】")
    ...
    md_file = os.path.join(dir_name, f"analysis_report_{timestamp}.md")
    
    with open(md_file, "w", encoding="utf-8") as f:
        f.write("\n".join(md_lines))
    print(f"\n📄 Markdown 分析报告已保存至: {md_file}")

# ==========================================
if __name__ == "__main__":
    import sys
    FILE_NAME = sys.argv[1] if len(sys.argv) > 1 else 'ReportTester-106832906-4-Deals.csv'
